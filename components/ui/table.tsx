...
const TableHeader = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(
  ({ className, ...props }, ref) => (
    <thead ref={ref} className={cn("bg-ledger text-ledger-foreground", className)} {...props} />
  )
);
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "h-11 whitespace-nowrap px-4 text-left align-middle font-display text-xs font-semibold uppercase tracking-wide text-ledger-foreground/90",
        className
      )}
      {...props}
    />
